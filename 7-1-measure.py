...
#initialization of leds and ports
def init(arr, mode):
    for pin in arr:
        GPIO.setup(pin, mode)
...
#convertation numbers from binary(list of 8 elements 1 and 0) to decimal and against
...
#show binary representation of value in adc
def show_led(value):
...
def main():
    ...
    try:
        ...
        while True:
            ...
            if (volt/3300 >= 0.92):
                break
        #condensator has stopped charging and began discharging
        GPIO.output(V_out, 0)
        while True:
            res = adc(dac, V_in)
            show_led(res)
